...
$(document).on('click', '.btn-item-edit', function () {
  ...
  $.post(module_url + '/ajax_get_item', { id: data_id }, function (d) {
    if (d.status) {
      ...
      $('input[name="code"]').val(d.data.code);
      $('input[name="name"]').val(d.data.name);

      // append item to the selectbox
      $('.main-form').find('#software_list').html('');

      var item_id_list    = d.data.software_list_id.split(',');
      var item_text_list  = d.data.software_list.split(',');
      for (let i = 0; i < item_id_list.length; i++) {
        const id    = item_id_list[i];
        const text  = item_text_list[i];

        $('.main-form').find('#software_list').append(`<option value="${id}">${text}</option>`);        
      }

      $('.main-form').find('#software_list').val(d.data.software_list_id.split(',')).change();
