...
class Account extends Management_Controller {

  private $module_path = 'master/account';
  ...
  public function index(){
    $data['extraJs'] 	= [
      ...
      'master/account_index.js'
    ];
    $data['page_title'] = "Master Account ID";
    $data['page_view'] 	= "V_account";
  ...
  function ajax_module_index(){
    ...
    $db_total = $this->M_account->get_count(null, $where, null, null, null, null, null, $like, 'id');
    $db_data 	= $this->M_account->get(null, $where, null, null, ['name' => 'asc'], $limit, $offset, $like, 'id, name');
  ...
  function ajax_post_form(){
    ...
    if ($this->form_validation->run()) {
        ...
        $data = [
          ...
          $db = $this->M_account->insert(null, $data);
        } else {
					// log
          $data += [
          ...
          $db = $this->M_account->update(null, ['id' => $post['id']], $data);
        }
	...
	function ajax_get_item(){
		...
		if ($this->form_validation->run()) {
			$post = $this->input->post();
			$db	 	= $this->M_account->get(null, ['id' => $post['id']]);
			if ($db) {
					$status = true;
					$data 	= $db[0];
			}
	...
	function ajax_delete_item(){
		...
		if($id = $this->input->post('id')){
			...
			$db = $this->M_account->update(null, ['id' => $id], $data);
			if ($db) { $status = true; }
		}
  ...
  function ajax_get_account(){
    ...
    $db = $this->M_account->get(null, $where, null, null, ['name' => 'asc'], 100, null, $like, "id, name");
    foreach($db as $i => $v){
