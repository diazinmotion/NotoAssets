...
class Brand extends Management_Controller {

  private $module_path = 'master/brand';
  ...
  public function index(){
    $data['extraJs'] 	= [
      ...
      'master/brand_index.js'
    ];
    $data['page_title'] = "Master Brand";
    $data['page_view'] 	= "V_brand";
  ...
  function ajax_module_index(){
    ...
    $db_total = $this->M_brand->get_count(null, $where, null, null, null, null, null, $like, 'id');
    $db_data 	= $this->M_brand->get(null, $where, null, null, ['name' => 'asc'], $limit, $offset, $like, 'id, name');
  ...
  function ajax_post_form(){
    ...
    if ($this->form_validation->run()) {
        ...
        $data = [
          ...
          $db = $this->M_brand->insert(null, $data);
        } else {
					// log
          $data += [
          ...
          $db = $this->M_brand->update(null, ['id' => $post['id']], $data);
        }
	...
	function ajax_get_item(){
		...
		if ($this->form_validation->run()) {
			$post = $this->input->post();
			$db	 	= $this->M_brand->get(null, ['id' => $post['id']]);
			if ($db) {
					$status = true;
					$data 	= $db[0];
			}
	...
	function ajax_delete_item(){
		...
		if($id = $this->input->post('id')){
			...
			$db = $this->M_brand->update(null, ['id' => $id], $data);
			if ($db) { $status = true; }
		}
  ...
  function ajax_get_brand(){
    ...
    $db = $this->M_brand->get(null, $where, null, null, ['name' => 'asc'], 100, null, $like, "id, name");
    foreach($db as $i => $v){
