...
class Os_type extends Management_Controller {

  private $module_path = 'master/os_type';

  function __construct(){
    parent::__construct();

    $this->load->model('M_os_type');
  }

  public function index(){
    $data['extraJs'] 	= [
      'statics/app_common.js',
      'master/os_type_index.js'
    ];
    $data['page_title'] = "Master Operating System";
    $data['page_view'] 	= "V_os_type";
  ...
  function ajax_module_index(){
    ...
    $db_total = $this->M_os_type->get_count(null, $where, null, null, null, null, null, $like, 'id');
    $db_data 	= $this->M_os_type->get(null, $where, null, null, ['name' => 'asc'], $limit, $offset, $like, 'id, name');
  ...
  function ajax_post_form(){
    ...
    if ($this->form_validation->run()) {
        ...
        $data = [
          ...
          $db = $this->M_os_type->insert(null, $data);
        } else {
					// log
          $data += [
          ...
          $db = $this->M_os_type->update(null, ['id' => $post['id']], $data);
        }
	...
	function ajax_get_item(){
		...
		if ($this->form_validation->run()) {
			$post = $this->input->post();
			$db	 	= $this->M_os_type->get(null, ['id' => $post['id']]);
			if ($db) {
					$status = true;
					$data 	= $db[0];
			}
	...
	function ajax_delete_item(){
		...
		if($id = $this->input->post('id')){
			...
			$db = $this->M_os_type->update(null, ['id' => $id], $data);
			if ($db) { $status = true; }
		}
  ...
  function ajax_get_os(){
    ...
    $db = $this->M_os_type->get(null, $where, null, null, ['name' => 'asc'], 100, null, $like, "id, name");
    foreach($db as $i => $v){
