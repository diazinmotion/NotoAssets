...
class Storage_type extends Management_Controller {

  private $module_path = 'master/storage_type';

  function __construct(){
    parent::__construct();

    $this->load->model('M_storage_type');
  }

  public function index(){
    $data['extraJs'] 	= [
      'statics/app_common.js',
      'master/storage_type_index.js'
    ];
    $data['page_title'] = "Master Storage Type";
    $data['page_view'] 	= "V_storage_type";
  ...
  function ajax_module_index(){
    ...
    $db_total = $this->M_storage_type->get_count(null, $where, null, null, null, null, null, $like, 'id');
    $db_data 	= $this->M_storage_type->get(null, $where, null, null, ['name' => 'asc'], $limit, $offset, $like, 'id, code, name');
  ...
  function ajax_post_form(){
    ...
    if ($this->form_validation->run()) {
        ...
        // cek apakah kode ini sudah ada?
				$exist = $this->M_storage_type->get(null, ['code' => $post['code'], 'deleted_at' => null]);
          ...
          if(!$exist){
            ...
            $db = $this->M_storage_type->insert(null, $data);
          }else{
						$msg = 'This code has been used. Please use another code';
          }
        } else {
          // cek apakah kode ini sudah ada?
					$current_data = $this->M_storage_type->get(null, ['id' => $post['id']]);
            ...
            $db = $this->M_storage_type->update(null, ['id' => $post['id']], $data);
          }else{
						$msg = 'This code has been used. Please use another code';
          }
        }
	...
	function ajax_get_item(){
		...
		if ($this->form_validation->run()) {
			$post = $this->input->post();
			$db	 	= $this->M_storage_type->get(null, ['id' => $post['id']]);
			if ($db) {
					$status = true;
					$data 	= $db[0];
			}
	...
	function ajax_delete_item(){
		...
		if($id = $this->input->post('id')){
			...
			$db = $this->M_storage_type->update(null, ['id' => $id], $data);
			if ($db) { $status = true; }
		}
  ...
  function ajax_get_storage(){
    ...
    $db = $this->M_storage_type->get(null, $where, null, null, ['name' => 'asc'], 100, null, $like, "id, code, name");
    foreach($db as $i => $v){
